...
SockMessages::SockMessages(void): SettingsMessagesBase(client_address + 1) {
  ...
  messages[client_address] = gettext("This determines whether addresses other than localhost and your machine's own "
				     ...
				     "numeric dot notation.  Numeric dot notation may have * as a trailing wildcard, "
				     "so for example 10.1.* will permit access for addresses 10.1.0.0 to 10.1.255.255.  "
...
void SettingsHelpDialogCB::settings_help_button_clicked(GtkWidget*, void* data) {
...
SettingsHelpDialog::SettingsHelpDialog(const int standard_size, const std::string& text,
  ...
  g_signal_connect(G_OBJECT(close_button_p), "clicked",
		   G_CALLBACK(SettingsHelpDialogCB::settings_help_button_clicked), this);
