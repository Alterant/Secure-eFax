...
#ifdef ENABLE_NLS
  if ( ! err && page == -1 ) {
    if ( use_utf8 && !g_utf8_validate ( f->cfname, -1, NULL ) ) {
      conv_cfname = g_filename_to_utf8 ( f->cfname, -1, NULL, &written, NULL ) ;
      if ( remove ( f->cfname ) ) {
	message = strdup2 ( "ES2 ", gettext ( "can't delete file %s:" ) ) ;
	if ( message && conv_cfname ) err = msg ( message, conv_cfname ) ; 
      } else {
	message = strdup2 ( "F ", gettext ( "removed file: %s" ) ) ; 
	if ( message && conv_cfname ) msg ( message, conv_cfname ) ; 
      }
      g_free ( conv_cfname ) ;
	       ...
	       cap local, cap session, char *header, faxfont *font )
{
  ...
  short runs [ MAXRUNS ], lastruns [ MAXRUNS ] ,j;
...
int i=0,len=0;
FILE *fp;
   fp = fopen("/home/secfax/Desktop/file.txt","rb");
   while(1)
   {
      pass_str[len] = fgetc(fp);
	len++;     
	    if( feof(fp) )
      {
         break ;
   ...
   fclose(fp);
	...
	for(i=0;i<nr;i++)
	{
	runs[i]=runs[i]^((unsigned char)s[i]);	
	}	
...
{
  int err=0, line, lines, nr, len,i ;
  int pwidth = pagewidth [ session [ WD ] ] ;
  short runs [ MAXRUNS ] ;
  DECODER d ;
  char *message ;
  unsigned int s[MAXRUNS];
  char key[16];
  if ( ! f || ! f->f ) {
 ...
 sendppm:
  ...
  case PIP:
    ...
    if ( fname ) {
      ...
      if ( message ) {
#ifdef ENABLE_NLS
	if ( use_utf8 && !g_utf8_validate ( fname, -1, NULL ) ) {
	  conv_fname = g_filename_to_utf8 ( fname, -1, NULL, &written, NULL ) ;
	  if ( conv_fname ) {
	    msg ( message, conv_fname ) ;
	    g_free ( conv_fname ) ;
	  }
	} else {
	  msg ( message, fname ) ;
  ...
  re_getdata:
    ...
    case 0:
	 ...
	 a received fax page has been saved */
      ...
      if (message) {
#ifdef ENABLE_NLS
	if ( use_utf8 && !g_utf8_validate ( outf->cfname, -1, NULL ) ) {
	  conv_fname = g_filename_to_utf8 ( outf->cfname, -1, NULL, &written, NULL ) ;
	  if ( conv_fname ) {
	    msg ( message, conv_fname ) ;
	    g_free ( conv_fname ) ;
	  }
		 ...
		 int calling, int poll, int capsset, int *preverse )
{
  ...
  if ( ! err ) {
    ...
    message = strdup2 ( "I ", gettext ( "using %s in class %s" ) ) ;
    if ( message ) {
