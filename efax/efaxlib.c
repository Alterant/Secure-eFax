...
#ifdef ENABLE_NLS
      if ( fn ) {
	if ( use_utf8 && !g_utf8_validate ( fn, -1, NULL ) ) {
	  fn = g_filename_to_utf8 ( fn, -1, NULL, &written, NULL ) ;
	  if ( fn ) {
	    msg ( s, fn ) ;
	    g_free ( fn ) ;
	  }
...
int newIFILE ( IFILE *f, char **fnames )
{
  ...
  for ( p=fnames ; ! err && *p ; p++ ) {
    ...
    if ( ! ( f->f = fopen ( *p, "rb" ) ) ) {
      ...
      if ( message ) {
#ifdef ENABLE_NLS
	if ( use_utf8 && !g_utf8_validate ( *p, -1, NULL ) ) {
	  conv_fname = g_filename_to_utf8 ( *p, -1, NULL, &written, NULL ) ;
	  if ( conv_fname ) {
	    err = msg ( message, conv_fname ) ;
	    g_free ( conv_fname ) ;
	  }
    ...
    if ( ! err ) {
      ...
      if ( ! fformat ) {
	message = strdup2 ( "E2 ", gettext ( "can't get format of file %s" ) ) ;
	if ( message ) {
#ifdef ENABLE_NLS
	  if ( use_utf8 && !g_utf8_validate ( *p, -1, NULL ) ) {
	    conv_fname = g_filename_to_utf8 ( *p, -1, NULL, &written, NULL ) ;
    ...
    if ( ! err && fseek ( f->f, 0, SEEK_SET ) ) {
      message = strdup2 ( "ES2 ", gettext ( "can't rewind file %s:" ) ) ;
      if ( message ) {
#ifdef ENABLE_NLS
	if ( use_utf8 && !g_utf8_validate ( *p, -1, NULL ) ) {
	  conv_fname = g_filename_to_utf8 ( *p, -1, NULL, &written, NULL ) ;
...
int nextopage ( OFILE *f, int page )
{
  ...
  if ( ! err && page >= 0 ) {	/* open new file */
	...
	f->f = freopen ( f->cfname, ( f->format == O_PS ) ? "w" : "wb+", f->f ) ;
      ...
      if ( ! f->f ) {
	message = strdup2 ( "ES2 ", gettext ( "can't open output file %s:" ) ) ;
	if ( message ) {
#ifdef ENABLE_NLS
	  if ( use_utf8 && !g_utf8_validate ( f->cfname, -1, NULL ) ) {
	    conv_cfname = g_filename_to_utf8 ( f->cfname, -1, NULL, &written, NULL ) ;
	    if ( conv_cfname ) {
	      err = msg ( message, conv_cfname ) ;
	      g_free ( conv_cfname ) ;
	    }
	  } else {
	    err = msg ( message, f->cfname ) ;
	  }
#else
	  err = msg ( message, f->cfname ) ;
#endif
